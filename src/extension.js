...
// Files currently being saved by the push handler — prevents the onDidSaveTextDocument
// watcher from triggering a second push on the same file.
const pushInProgress = new Set();
let client = null;
...
function activate(context) {
            ...
            async () => {
                try {
                    ...
                    if (libraryRow.Frozen === true) {
                        const reason = libraryRow.Published
                            ? `${libraryId} is promoted to production — demote it first (right-click the library → Demote from Production).`
                            : `${libraryId} is locked/frozen in Epicor. Unfreeze it before editing.`;
            ...
            async () => {
                try {
                    ...
                    if (libraryRow.Frozen === true) {
                        const reason = libraryRow.Published
                            ? `${libraryId} is promoted to production — demote it first (right-click the library → Demote from Production).`
                            : `${libraryId} is locked/frozen in Epicor. Unfreeze it before deleting functions.`;
                        throw new Error(reason);
                    }
    ...
    // ── Set Table Read-only / Updatable ──
    context.subscriptions.push(vscode.commands.registerCommand("efx.setTableUpdatable", async (node) => {
        if (!client) { vscode.window.showWarningMessage("EFx: Configure connection first"); return; }
        if (!node || !node.row) { vscode.window.showWarningMessage("EFx: Must be invoked from a table in the tree."); return; }
        const libraryId = node.row.LibraryID;
        const tableId   = node.row.TableID;
        const current   = node.row.Updatable;
        if (!libraryId || !tableId) { vscode.window.showErrorMessage("EFx: Could not determine library/table from selection."); return; }
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(async (doc) => {
        ...
        // Skip if the save was triggered by our own push handler (prevents double-push).
    ...
    context.subscriptions.push(vscode.commands.registerCommand("efx.bpm.openWidgetPanel", async (nodeOrArg) => {
        ...
        if (!directive) { vscode.window.showWarningMessage('BPM: Select a directive'); return; }
    ...
    // (used by stashed Kinetic commands — kept so uncommenting them works)
    ...
    // ── Kinetic App Commands (stashed — uncomment to re-enable) ──────────────
    /*
    context.subscriptions.push(vscode.commands.registerCommand("kinetic.transferApp", async () => {
        ...
        const appIdRaw = await vscode.window.showInputBox({ prompt: "App ID to transfer. Leave blank to browse available apps.", placeHolder: "Leave blank to see a list", ignoreFocusOut: true });
        if (appIdRaw === undefined) return;
        let viewId = appIdRaw.trim();
        if (!viewId) {
            let allApps;
            try { allApps = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Fetching app list…" }, () => sourceClient.listApps()); }
            catch (err) { vscode.window.showErrorMessage(`Kinetic: Failed to list apps: ${err.message}`); return; }
            if (allApps.length === 0) { vscode.window.showWarningMessage("Kinetic: No apps found on source."); return; }
            const appPick = await vscode.window.showQuickPick(allApps.map(a => ({ label: a.Id, description: a.Type || "", detail: a.SubType || "", id: a.Id })), { placeHolder: "Select app to transfer", ignoreFocusOut: true });
            if (!appPick) return;
            viewId = appPick.id;
        }
        let appContent;
        try { appContent = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Exporting ${viewId}…` }, () => sourceClient.exportApp(viewId)); }
        catch (err) { vscode.window.showErrorMessage(`Kinetic: Export failed: ${err.message}`); return; }
        if (!appContent) { vscode.window.showWarningMessage(`Kinetic: App "${viewId}" not found on source.`); return; }
        const targetClient = await pickKineticEnv("Target environment (import app to)");
        if (!targetClient) return;
        let exists = false;
        try { exists = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Checking if "${viewId}" exists on target…` }, () => targetClient.applicationExists(viewId)); } catch { }
        if (exists) { const confirm = await vscode.window.showWarningMessage(`App "${viewId}" already exists on the target. Overwrite it?`, { modal: true }, "Overwrite"); if (confirm !== "Overwrite") return; }
        try { await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Saving ${viewId} to target…` }, () => targetClient.saveApp(viewId, appContent.Files)); }
        catch (err) { vscode.window.showErrorMessage(`Kinetic: Save failed: ${err.message}`); return; }
        try { await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Publishing ${viewId} on target…` }, () => targetClient.publishApp(viewId, appContent.Files)); vscode.window.showInformationMessage(`Kinetic: App "${viewId}" transferred successfully.`); }
        catch (err) { vscode.window.showErrorMessage(`Kinetic: Publish failed: ${err.message}`); }
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand("kinetic.transferLayers", async () => {
        ...
        if (!sourceClient) return;
        const appIdRaw = await vscode.window.showInputBox({ prompt: "App ID whose layers you want to transfer", placeHolder: "e.g. Erp.UI.SalesOrderEntry", ignoreFocusOut: true });
        if (appIdRaw == null || !appIdRaw.trim()) return;
        const viewId = appIdRaw.trim();
        const layerNamesInput = await vscode.window.showInputBox({ prompt: "Layer names to transfer (comma-separated). Leave blank to pick from a list.", placeHolder: "e.g. Company-EPIC, Site-MAIN  — or leave blank to browse", ignoreFocusOut: true });
        if (layerNamesInput === undefined) return;
        const requestedNames = layerNamesInput.split(",").map(s => s.trim()).filter(Boolean);
        let allLayers;
        try { allLayers = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Fetching layers for ${viewId}…` }, () => sourceClient.getLayers(viewId)); }
        catch (err) { vscode.window.showErrorMessage(`Kinetic: Failed to fetch layers: ${err.message}`); return; }
        if (allLayers.length === 0) { vscode.window.showWarningMessage(`Kinetic: No layers found for app "${viewId}" on source.`); return; }
        let selectedLayers;
        if (requestedNames.length > 0) {
            ...
            if (missing.length > 0) vscode.window.showWarningMessage(`Kinetic: Layer(s) not found on source: ${missing.join(", ")}`);
            if (selectedLayers.length === 0) return;
        } else {
            const picks = await vscode.window.showQuickPick(allLayers.map(l => ({ label: l.LayerName || l.Id || "(unnamed)", description: l.TypeCode || "", detail: `Device: ${l.DeviceType || ""}  Published: ${l.IsPublished ? "Yes" : "No"}`, layer: l, picked: true })), { canPickMany: true, placeHolder: "Select layers to transfer", ignoreFocusOut: true });
            if (!picks || picks.length === 0) return;
            selectedLayers = picks.map(p => p.layer);
        }
        let exportedContent;
        try { exportedContent = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Exporting ${selectedLayers.length} layer(s)…` }, () => sourceClient.exportLayers(selectedLayers)); }
        catch (err) { vscode.window.showErrorMessage(`Kinetic: Export failed: ${err.message}`); return; }
        if (exportedContent == null) { vscode.window.showWarningMessage("Kinetic: Export returned empty — nothing to import."); return; }
        const targetClient = await pickKineticEnv("Target environment (import layers to)");
        if (!targetClient) return;
        try { await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Importing ${selectedLayers.length} layer(s) to target…` }, () => targetClient.importLayers(exportedContent, true)); vscode.window.showInformationMessage(`Kinetic: ${selectedLayers.length} layer(s) imported successfully.`); }
        catch (err) { vscode.window.showErrorMessage(`Kinetic: Import failed: ${err.message}`); }
    }));

    context.subscriptions.push(vscode.commands.registerCommand("kinetic.pullApp", async () => {
        const sourceClient = await pickKineticEnv("Environment to pull app from");
        if (!sourceClient) return;
        const appIdRaw = await vscode.window.showInputBox({ prompt: "App ID to pull. Leave blank to browse.", placeHolder: "e.g. Erp.UI.SalesOrderEntry — or leave blank", ignoreFocusOut: true });
        if (appIdRaw === undefined) return;
        let viewId = appIdRaw.trim();
        if (!viewId) {
            let allApps;
            ...
            if (allApps.length === 0) { vscode.window.showWarningMessage("Kinetic: No apps found."); return; }
            const appPick = await vscode.window.showQuickPick(allApps.map(a => ({ label: a.Id, description: a.Type || "", detail: a.SubType || "", id: a.Id })), { placeHolder: "Select app to pull", ignoreFocusOut: true });
            if (!appPick) return;
            viewId = appPick.id;
        }
        let appContent;
        try { appContent = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Pulling ${viewId}…` }, () => sourceClient.exportApp(viewId)); }
        catch (err) { vscode.window.showErrorMessage(`Kinetic: Pull failed: ${err.message}`); return; }
        if (!appContent || !appContent.Files) { vscode.window.showWarningMessage(`Kinetic: App "${viewId}" returned no files.`); return; }
        const kineticDir = getKineticDir();
        const appDir = path.join(kineticDir, viewId);
        fs.mkdirSync(appDir, { recursive: true });
        for (const [filename, content] of Object.entries(appContent.Files)) {
            if (!content) continue;
            const safeName = (filename.endsWith(".json") || filename.endsWith(".jsonc")) ? filename : filename + ".json";
            const filePath = path.join(appDir, safeName);
            try { fs.writeFileSync(filePath, JSON.stringify(JSON.parse(content), null, 2), "utf-8"); } catch { fs.writeFileSync(filePath, content, "utf-8"); }
        }
        fs.writeFileSync(path.join(appDir, "_meta.json"), JSON.stringify({ viewId, serverUrl: sourceClient.config.serverUrl, company: sourceClient.config.company, pulledAt: new Date().toISOString() }, null, 2), "utf-8");
        const layoutPath = path.join(appDir, "layout.json");
        if (fs.existsSync(layoutPath)) { const doc = await vscode.workspace.openTextDocument(layoutPath); await vscode.window.showTextDocument(doc, vscode.ViewColumn.One); }
        vscode.window.showInformationMessage(`Kinetic: Pulled "${viewId}" — edit files in .kinetic/${viewId}/, then Push App when ready.`);
    }));

    context.subscriptions.push(vscode.commands.registerCommand("kinetic.pushApp", async () => {
        const kineticDir = getKineticDir();
        let viewId = null, appDir = null;
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            const filePath = editor.document.uri.fsPath;
            if (filePath.startsWith(kineticDir + path.sep)) {
                const parts = path.relative(kineticDir, filePath).split(path.sep);
                if (parts.length >= 1) { viewId = parts[0]; appDir = path.join(kineticDir, viewId); }
            }
        }
        if (!appDir || !viewId) { vscode.window.showWarningMessage("Kinetic: Open a file from a pulled app folder first (Pull App to get one)."); return; }
        const files = {};
        for (const filename of fs.readdirSync(appDir)) {
            if (filename === "_meta.json") continue;
            try { files[filename] = fs.readFileSync(path.join(appDir, filename), "utf-8"); } catch { }
        }
        if (Object.keys(files).length === 0) { vscode.window.showErrorMessage(`Kinetic: No app files found in .kinetic/${viewId}/`); return; }
        const targetClient = await pickKineticEnv("Target environment to push app to");
        if (!targetClient) return;
        let exists = false;
        try { exists = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Checking if "${viewId}" exists on target…` }, () => targetClient.applicationExists(viewId)); } catch { }
        if (exists) { const confirm = await vscode.window.showWarningMessage(`App "${viewId}" already exists on the target. Overwrite?`, { modal: true }, "Overwrite"); if (confirm !== "Overwrite") return; }
        try { await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Saving ${viewId} to target…` }, () => targetClient.saveApp(viewId, files)); }
        catch (err) { vscode.window.showErrorMessage(`Kinetic: Save failed: ${err.message}`); return; }
        try { await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Publishing ${viewId} on target…` }, () => targetClient.publishApp(viewId, files)); vscode.window.showInformationMessage(`Kinetic: App "${viewId}" pushed and published ✓`); }
        catch (err) { vscode.window.showErrorMessage(`Kinetic: Publish failed: ${err.message}`); }
    }));
    */
    // ── DIAGNOSTIC: dump widget XAML for a BPM directive ──────────────────────
    context.subscriptions.push(vscode.commands.registerCommand("efx.dev.dumpBpmXaml", async () => {
        ...
        try {
            ...
            for (const d of dirs) {
                ...
                if (!d.Body) { out.appendLine("  (no body)"); continue; }

                // Find all action element types present
                ...
                // Find all CustomCodeAction Code= values
                ...
                while (true) {
                    ...
                    while (valueEnd < d.Body.length) {
                        ...
                        if (d.Body[valueEnd] === '&') { const s = d.Body.indexOf(';', valueEnd); if (s < 0) break; valueEnd = s + 1; continue; }
                        valueEnd++;
                    }
...
function getKineticDir() {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (workspaceFolders && workspaceFolders.length > 0) {
        return path.join(workspaceFolders[0].uri.fsPath, ".kinetic");
    }
    const homeDir = process.env.HOME || process.env.USERPROFILE || "/tmp";
    return path.join(homeDir, ".kinetic");
}
